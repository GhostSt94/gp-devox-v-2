...
public class Facture extends AbstractVerticle {

    private final Logger logger = Logger.getLogger(Facture.class);
    private final String COLLECTION_NAME = Collections.FACTURE;

    @Override
    public void start(Promise<Void> startPromise) throws Exception {

        try {
            vertx.eventBus().consumer(Services.FACTURE_LIST, this::listWithAttachement);
            vertx.eventBus().consumer(Services.FACTURE_CREATE, this::create);
            vertx.eventBus().consumer(Services.FACTURE_MODIFY, this::modify);
            vertx.eventBus().consumer(Services.FACTURE_REMOVE, this::remove);
            vertx.eventBus().consumer(Services.FACTURE_STATS, this::stats);
    ...
    private void list(Message<JsonObject> message) {
        try {
            logger.debug("consumer -service `" + Services.FACTURE_LIST + "`");
    ...
    private void listWithAttachement(Message<JsonObject> message) {
        try {
            logger.debug("consumer -service `" + Services.FACTURE_LIST + "`");
            ...
            JsonArray countFactures = new JsonArray()
                    .add(new JsonObject().put("$match",query))
                    .add(new JsonObject().put("$count","count"));

            JsonArray docs=new JsonArray().add(new JsonObject().put("$match",query));

            if(options.containsKey("page")) {
                int page = options.getInteger("page", 0);
                page = page != 0 ? page - 1 : page;

                docs.add(new JsonObject().put("$skip", page * options.getInteger("limit")));
            }

            if(options.containsKey("limit")) docs.add(new JsonObject()
                    .put("$limit", options.getInteger("limit")));


            docs.add(new JsonObject()
                    .put("$lookup",new JsonObject()
                            .put("from", Collections.ATTACHEMENT)
                            .put("localField", "_id")
                            .put("foreignField", Fields.ATTACHMENT_ID_FACTURE)
                            .put("as", "attachements")));
            ...
            JsonArray pipeline  = new JsonArray()
                    .add(new JsonObject()
                            .put("$facet", new JsonObject()
                                    .put("countFactures", countFactures)
                                    .put("docs", docs)));
            ...
            vertx.eventBus().request(Services.DB_RUN_COMMAND, msg, ar -> defaultHandler(ar, message));
    ...
    private void create(Message<JsonObject> message) {
        try {
            logger.debug("consumer -service `" + Services.FACTURE_CREATE + "`");
    ...
    private void modify(Message<JsonObject> message) {
        try {
            logger.debug("consumer -service `" + Services.FACTURE_MODIFY + "`");
    ...
    private void remove(Message<JsonObject> message) {
        try {
            logger.debug("consumer -service `" + Services.FACTURE_REMOVE + "`");

            JsonObject body = message.body();

            String _id = body.getString("_id");

            if(body.containsKey("attachement"))
                vertx.eventBus().request(Services.ATTACHEMENT_REMOVE, body.getJsonObject("attachement"));
