...
public class Client extends AbstractVerticle {
    ...
    private final String COLLECTION_NAME = Collections.CLIENT;

    @Override
    public void start(Promise<Void> startPromise) throws Exception {

        try {
            vertx.eventBus().consumer(Services.CLIENT_LIST, this::list);
            vertx.eventBus().consumer(Services.CLIENT_CREATE, this::create);
            vertx.eventBus().consumer(Services.CLIENT_MODIFY, this::modify);
            vertx.eventBus().consumer(Services.CLIENT_REMOVE, this::remove);
    ...
    private void list(Message<JsonObject> message) {
        try {
            logger.debug("consumer -service `" + Services.CLIENT_LIST + "`");
    ...
    private void create(Message<JsonObject> message) {
        try {
            logger.debug("consumer -service `" + Services.CLIENT_CREATE + "`");
    ...
    private void modify(Message<JsonObject> message) {
        try {
            logger.debug("consumer -service `" + Services.CLIENT_MODIFY + "`");
    ...
    private void remove(Message<JsonObject> message) {
        try {
            logger.debug("consumer -service `" + Services.CLIENT_REMOVE + "`");
