...
public class Attachement extends AbstractVerticle {

	private final Logger logger = Logger.getLogger(Attachement.class);
	private final String COLLECTION_NAME = Collections.ATTACHEMENT;

	@Override
	public void start(Promise<Void> startPromise) throws Exception {

		try {
			vertx.eventBus().consumer(Services.ATTACHEMENT_LIST, this::list);
			vertx.eventBus().consumer(Services.ATTACHEMENT_CREATE, this::create);
			vertx.eventBus().consumer(Services.ATTACHEMENT_REMOVE, this::remove);
	...
	private void list(Message<JsonObject> message) {
		try {
			logger.debug("consumer -service `" + Services.ATTACHEMENT_LIST + "`");
	...
	private void create(Message<JsonObject> message) {
		try {
			logger.debug("consumer -service `" + Services.ATTACHEMENT_CREATE + "`");
	...
	private void remove(Message<JsonObject> message) {
		try {
			logger.debug("consumer -service `" + Services.ATTACHEMENT_REMOVE + "`");
			...
			if(body.containsKey("path"))
				vertx.eventBus().request(Services.REMOVE_UPLOAD, body);
